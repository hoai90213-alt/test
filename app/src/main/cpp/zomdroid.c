...
static void* zomdroid_loader_dlopen_shim(const char* filename, int flags, const void* caller) {
    (void)caller;
    return dlopen(filename, flags);
}

static void* zomdroid_loader_dlsym_shim(void* handle, const char* symbol, const void* caller) {
    (void)caller;
    return dlsym(handle, symbol);
}

static void* zomdroid_loader_android_dlopen_ext_shim(const char* filename, int flags,
...
static const char* zomdroid_pick_jvm_lib_name() {
#if defined(__APPLE__)
    return "libjvm.dylib";
#else
    return "libjvm.so";
...
static void create_jvm_and_launch_main(int jvm_argc, const char** jvm_argv, const char* main_class_name, int argc, const char** argv) {
    ...
    void* libjvm = linkernsbypass_namespace_dlopen(zomdroid_pick_jvm_lib_name(), RTLD_GLOBAL, zomdroid_ns);
#else
    const char* jvm_lib_override = getenv("ZOMDROID_JVM_LIB");
    const char* jvm_lib_name = zomdroid_pick_jvm_lib_name();
    char jvm_lib_path[PATH_MAX];
    if (jvm_lib_override && jvm_lib_override[0]) {
    ...
    if (JNI_CreateJavaVM == NULL) {
        LOGE("Failed to locate JNI_CreateJavaVM: %s", dlerror());
        return;
    }
...
static int init_zomdroid_namespace(const char* ld_library_path) {
    ...
    if (ld_library_path && ld_library_path[0]) {
        setenv("ZOMDROID_LIBRARY_DIR", ld_library_path, 1);
    }
    zomdroid_ns = NULL;
#endif
    return 0;
...
static int load_linker_hook() {
...
#else
    const char* linker_lib_override = getenv("ZOMDROID_LINKER_LIB");
    const char* linker_lib_name = NULL;
    char linker_lib_name_buf[PATH_MAX];
    char linker_lib_name_default[64];
    ...
    void* zomdroid_linker = dlopen(linker_lib_name, RTLD_LOCAL | RTLD_NOW);
#endif
    if (!zomdroid_linker) {
        LOGE("%s", dlerror());
        return -1;
    ...
    if (!zomdroid_linker_init || !zomdroid_linker_set_proc_addrs ||
        ...
        dlerror();
        LOGE("Failed to locate symbols for libzomdroidlinker");
        return -1;
    }
...
#else
    void* _loader_dlopen_fn = (void*)&zomdroid_loader_dlopen_shim;
    void* _loader_dlsym_fn = (void*)&zomdroid_loader_dlsym_shim;
    void* _loader_android_dlopen_ext_fn = (void*)&zomdroid_loader_android_dlopen_ext_shim;
#endif
...
#else
        const char* vulkan_loader_name = getenv("ZOMDROID_VULKAN_LOADER_NAME");
        if (vulkan_loader_name == NULL || vulkan_loader_name[0] == '\0') {
            vulkan_loader_name = "libvulkan.1.dylib";
        }
        void* vulkan_loader = dlopen(vulkan_loader_name, RTLD_LOCAL | RTLD_NOW);
#endif
        if (!vulkan_loader) {
            LOGE("%s", dlerror());
            return -1;
...
#else
        void* vulkan_driver = dlopen(g_zomdroid_vulkan_driver_name, RTLD_LOCAL | RTLD_NOW);
#endif
        if (!vulkan_driver) {
            LOGE("%s", dlerror());
            return -1;
...
#define ENQUEUE_EVENT(setup_code)                                     \
    do {                                                              \
        unsigned char head = atomic_load_explicit(&g_zomdroid_event_queue.head, memory_order_relaxed);  \
        unsigned char tail = atomic_load_explicit(&g_zomdroid_event_queue.tail, memory_order_acquire);  \
        unsigned char next = (head + 1) & EVENT_QUEUE_MAX;                   \
        if (next == tail) {                                           \
            break;                                                    \
        }                                                             \
